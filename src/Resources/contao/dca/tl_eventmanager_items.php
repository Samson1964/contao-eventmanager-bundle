	...
	(
		...
		(
			...
			(
				...
				'button_callback'     => array('tl_eventmanager_items', 'toggleIcon')
			),
			'show' => array
			(
...
class tl_eventmanager_items extends Backend
{
	...
	public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_eventmanager_items::published', 'alexf'))
		{
			return '';
		}
	...
	public function toggleVisibility($intId, $blnPublished)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_eventmanager_items::published', 'alexf'))
		{
			$this->log('Kein Zugriffsrecht für Aktivierung Datensatz ID "'.$intId.'"', 'tl_eventmanager_items toggleVisibility', TL_ERROR);
		...
		$this->createInitialVersion('tl_eventmanager_items', $intId);
		...
		// Update the database
		$this->Database->prepare("UPDATE tl_eventmanager_items SET tstamp=". time() .", published='" . ($blnPublished ? '' : '1') . "' WHERE id=?")
		               ->execute($intId);
		$this->createNewVersion('tl_eventmanager_items', $intId);
	}
